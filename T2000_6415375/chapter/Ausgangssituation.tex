...
\section{Monitoring über MQTT.fx}
    ...
    \subsection{Funktionalität von MQTT.fx}
        ...
        Der Funktionsumfang ist dabei bewusst auf die Rolle eines Client-Werkzeugs ausgelegt, wobei der produktive Einsatz als dauerhaftes Monitoring- oder Visualisierungssystem nicht intendiert ist. Dadurch kann \textit{MQTT.fx} ein breites Publikum ansprechen, hat jedoch keine genaueren Analysemittel für genauere Anwendungen \autocite[13--14]{brooksNoSilverBullet1987}.
    \subsection{Gründe für und gegen den Einsatz von MQTT.fx}
        Diese Applikation ermöglicht es Entwicklern, ohne eigene Implementierung Publisher- und Subscriber-Rollen zu simulieren. 
         Ein klarer Vorteil bei MQTT.fx ist die Möglichkeit, Nachrichtenverläufe durch selbst erstellte Java-Skripte zu senden. Dadurch können jederzeit unterschiedliche Szenarien simuliert werden. Darüber hinaus bietet MQTT.fx eine chronologische Nachrichtenanzeige, wodurch falsche Zustandsannahmen 
        oder Timing-Probleme von Nachrichten erkannt werden können.
       ...
       eingeschränkt und erschwert die Analyse komplexer Topic-Hierarchien, wie sie in Automatisierungssystemen häufig auftreten.
        ...
        in ein bestehendes \gls{acr:HMI} oder produktive Visualisierungssysteme nicht möglich. Außerdem gibt es keine Möglichkeit zur Weiterverarbeitung von Daten, 
...
\subsection{Gründe gegen die Erweiterung von MQTTMultimeter}
    Während diese Applikation durch das Speichern von Nachrichten und Nachrichtenvorlagen sowie die Darstellung der Nachrichten sowohl in zeitlicher Reihenfolge als auch in Baumstruktur viele der Anforderungen an das Monitoring-Tool erfüllt, erweist sich der Programmcode als nur schwer erweiterbar. Bei sehr hohen Nachrichtenfrequenzen gelingt es ihr nicht, alle Nachrichten anzuzeigen und sie trennt sogar die Verbindung zum Broker. Die dafür erforderlichen Anpassungen würden umfangreiche Änderungen an bereits etablierten Komponenten notwendig machen. Da die Systemkomplexität mit fortschreitender Evolution eines Softwaresystems tendenziell zunimmt, steigt gleichzeitig der Aufwand für Analyse, Wartung un Weiterentwicklung \autocite{milanovicLehmansLawsSoftware}. In der Forschung zu Technical Dept \autocite{avgeriouTechnicalDebtManagement2023} wird darauf hingewiesen, dass hohe Komplexität und strukturelle Defizite die Fehleranfälligkeit erhöhen und zukünftige Entwicklungsaktivitäten erschweren können. Vor diesem Hintergrund wird die Weiterentwicklung der bestehenden Anwendung verworfen und stattdessen eine Neuentwicklung einer Monitoring- und Diagnose-Applikation gewählt.
...
\begin{table}[htbp] 
    ...
    \begin{tabular}{lccccc} 
        ...
        \midrule 
        Persistente Speicherung & 25\% & 1 & 1 & 2 \\ 
        Nachrichtenanzeige & 25\% & 3 & 3 & 4 \\
        Volltextsuche & 25\% & 1 & 5 & 3 \\ 
        Erweiterbarkeit & 15\% & 1 & 1 & 3 \\ 
        Simulationsmöglichkeiten & 5\% & 3 & 1 & 2 \\ 
        Performance & 5\% & 4 & 4 & 2 \\ 
        \midrule Gesamt & 100\% & 1,75 & 2,65 & 2,9 \\ 
        \bottomrule 
    \end{tabular} 
\end{table}
