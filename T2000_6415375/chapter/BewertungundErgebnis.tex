...
Die definierten Performance-Ziele konnten nicht vollständig erreicht werden. Insbesondere die CPU-Auslastung überschritt die festgelegten Grenzwerte während mehrerer Testläufe. Ursache hierfür war sowohl die bewusst hohe und kontinuierliche Nachrichtenlast während der Messungen als auch die rechenintensiven Einstellungen mit vier Nachrichtenfenstern auf einer Seite sowie das Laden von 60 Nachrichten pro Nachrichtenfenster. Das WPF-Framework verwendet für die Datenbindung zusätzliche Mechanismen zur Auflösung und Aktualisierung der gebundenen Datenobjekte, wodurch bei großen Datenmengen ein zusätzlicher Verarbeitungsaufwand entsteht \autocite{adegeoOptimizingPerformanceData}. Durch das verringern von den zu ladenden Nachrichten und gleichzeitig dargestellten Nachrichtenfenstern wird auch der Verarbeitungsaufwand dieser Datenbindungen entfernt. Dadurch könnte die CPU-Auslastung drastisch reduziert werden. Die Anwendung blieb trotz dieser rechenintensiven Einstellungen jederzeit stabil und konnte alle Nachrichten ohne Datenverluste verarbeiten. 

Neben der überschrittenen CPU-Auslastung ist auch der Arbeitsspeicherverbrauch zwei- bis dreimal höher als der geforderte Grenzwert. Auch bei mehreren Tests pendelt sich der Verbrauch wie bei Abbildung \ref{fig:Messwerte} ungefähr bei 1200 Megabyte ein. Dieses Phänomen ist dem sogenannten \textit{Garbagecollector} des \textit{.NET}-Frameworks geschuldet. Dieser ist für das Entfernen von nicht mehr referenzierten Objekten und toten Instanzen von Referenztypen aus dem Arbeitsspeicher zuständig, die nicht mehr gebraucht werden \autocite[22]{garbagecollector}. Da der Garbage Collector aber erst vom Allokator bei fehlendem Speicherplatz getriggert wird, arbeitet dieser nicht ständig, wodurch der Arbeitsspeicherverbrauch der Applikation zunächst kontinuierlich ansteigt \autocite[372--375]{garbagecollector}. Das permanente Entfernen von Objekten würde außerdem Performance-Einbuße mit sich bringen \autocite[372]{garbagecollector}. Für ein tieferes Verständnis für den Garbage Collector kann \autocite{garbagecollector} herangezogen werden.

Die ursprünglich festgelegten Grenzwerte erweisen sich dadurch als ambitionierter als für den praktischen Einsatz erforderlich. Die Messergebnisse zeigen zwar Optimierungspotenzial hinsichtlich Ressourcenverbrauch und Speicherverwaltung, stellen jedoch keine funktionale Einschränkung der Anwendung dar. Der erhöhte Arbeitsspeicherverbrauch wird zusätzlich durch das Speicherverwaltungsverhalten des \textit{.NET}-Frameworks beeinflusst \autocite[135]{garbagecollector}. Eine Optimierung dieser Teilbereiche stellt daher einen möglichen Ansatz für zukünftige Weiterentwicklungen dar. 
...
\begin{table}[htbp] 
    ...
    \begin{tabular}{lccccc} \hline \textbf{Kriterium} & \textbf{MQTT.fx} & \textbf{Explorer} & \textbf{Multimeter} & \textbf{Analyzer} \\ 
        \hline Nachrichtenanzeige (5\%) & 3 & 3 & 4 & 5 \\ 
        ...
        \hline Gesamtpunktzahl & 1.7 & 2.25 & 2.1 & 4.6 \\ 
        \hline 
    \end{tabular}
\end{table}
