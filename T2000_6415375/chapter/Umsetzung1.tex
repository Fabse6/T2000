...
Die entwickelte Anwendung umfasst in Summe 57 unterschiedliche Klassen, die jeweils in eigenen Programmdateien implementiert wurden. Die Klassen sind dabei in die Namensräume \textit{View}, \textit{ViewModel}, \textit{Model}, \textit{Service} und \textit{Repository} unterteilt. Die Namensräume \textit{View} und \textit{ViewModel} beinhalten die Klassen für die Benutzeroberfläche, während die Namensräume \textit{Service} und \textit{Repository} die Geschäftslogik und Persistenz der Anwendung abbilden. Die \textit{Models} enthalten die Datenstrukturen, die für eine einheitliche Datenhaltung innerhalb der Anwendung sorgen. Dadurch konnte eine klare Trennung zwischen Darstellung, Geschäftslogik und Persistenz erreicht werden. Insgesamt wurde die Anwendung in verschiedenen Klassen und XAML-Dateien implementiert.
...
Zur Verringerung des Implementierungsaufwands wurde das \textit{CommunityToolkit.MVVM} eingesetzt. Dadurch konnten Eigenschaften und Befehle automatisiert erzeugt werden, wodurch sich der manuell zu implementierende Programmcode deutlich reduzierte. \autocite{sergio0694EinfuehrungMVVMToolkitCommunity}

\section{Verarbeitung eingehender MQTT-Nachrichten}
Der Empfang der MQTT-Nachrichten sowie auch der Verbindungsaufbau, Verbindungsabbau und Erkennung von Verbindungsunterbrechungen wurde innerhalb einer zentralen MQTT-Serviceklasse realisiert. Diese Funktionen werden über die \textit{MQTTnet}-Bibliothek implementiert \autocite{teamHowUseMQTT}. Nach erfolgreicher Verbindung zum Broker werden alle Topics durch die Wildcard ,,\#'' abonniert. Bei einer neuen Nachricht wird das bibliothekseigene Event \lstinline{ApplicationMessageReceivedAsync} ausgelöst, wodurch Topic, Payload sowie die Retain-Flag der Nachricht ausgelesen werden kann \autocite{teamHowUseMQTT}. In der MQTT-Serviceklasse registriert sich ein Eventhandler auf dieses Event. Der Eventhandler löst daraufhin ein weiteres Event aus, dem sowohl Topic und Payload als auch die Retain-Flag der eingehenden Nachricht übergeben wird.
...
Vor der Speicherung werden empfangene Payloads von den verarbeitenden Komponenten zunächst als JSON-Dokument durch die \textit{System.Text.Json}-Bibliothek geparst. In \autocite{gewarrenHowDeserializeJSON} finden sich nähere Informationen zu der Nutzung der Bibliothek. Durch das Parsen stehen die Nachrichten in einer lesbaren Form zur Verfügung und können gleichzeitig als Grundlage für die Fehlererkennung, Prozessanalyse und Teilnehmerüberwachung verwendet werden.
...
Eine zentrale Anforderung der Anwendung bestand in der langfristigen Speicherung von MQTT-Nachrichten. Hierfür wurde eine SQLite-Datenbank eingesetzt \autocite{ajcvickersOverviewMicrosoftDataSqlite}. Neben den Kommunikationsdaten werden auch Fehler, Prozessabläufe, Nachrichtenvorlagen sowie Benutzereinstellungen in der Datenbank gespeichert. Dadurch können diese Informationen über mehrere Programmsitzungen hinweg erhalten bleiben und verbrauchen weniger Arbeitsspeicherplatz \autocite[2--4]{arulrajMultiTierBufferManagement2019}.
...
Die Benutzeroberfläche wurde mit WPF umgesetzt. Die Darstellung der Informationen erfolgt dabei durch eine Hauptansicht, in der die verschiedenen Unteransichten über ein Menü ausgewählt werden können. 

Neben dem Menü finden sich oben und unten Statusanzeigen wieder. Diese zeigen Informationen über alle Teilnehmer und über allgemeine Informationen wie interne Fehler der Anwendung oder die empfangene Nachrichtenanzahl. 
